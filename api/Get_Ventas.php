...
// intentar tu conexión
$path = __DIR__ . '/conexion2.php';
if (file_exists($path)) { require_once $path; }

// fallback PDO si no existe $pdo
if (!isset($pdo)) {
  try {
...
try {
  if (function_exists('consultaSQL')) {
    $rows = consultaSQL("Suc1, Suc2, Suc3, Suc4, Suc5", "venta", "");
  } else {
    $st = $pdo->query("SELECT Suc1, Suc2, Suc3, Suc4, Suc5 FROM venta");
    $rows = $st->fetchAll();
  }
